...
if($numoflist<=10){	//如果总数小于等于10则全部生成
 // makeList($get_data);
  ...
  $sql ="select * from personalinformation_records WHERE 'check_rsk'='uncheck' ORDER BY id DESC limit $limitHead,$limitEnd";//查询
...
foreach($query_sql as $row){
  ...
  echo "
  <form name='input' action='check_rsk.php' target='_self' method='get'>";
  echo "<td>" . $num . "</td>";
  ...
  if ($row['check_ywk']=="checked")//医务科审核通过
  {
	  echo "<td>" . "<input type='submit' id='checked' value='通过'/>"  . "</td>";

    echo "<input type='text' name='value' hidden='true' value='uncheck' />";
  }else if($row['check_ywk']=="tuihui"){//如果来自医务科的退回
    echo "<td>" . "<input type='submit' id='checked' style='color:red;' value='医务科未通过'/>"  . "</td>";
    echo "<input type='text' name='value' hidden='true' value='checked' />";
  }else{//人事科未审核
	  echo "<td>" . "<input type='submit' id='uncheck' value='点击审核'/>"  . "</td>";
    echo "<input type='text' name='value' hidden='true' value='checked' />";
    echo "</form>";
    //echo "<form name='input' action='check_rsk.php' target='_self' method='get'>";
    echo "<input type='text' name='id' hidden='true' value='{$row['id']}'/>";
    echo "<td>" . "<input type='submit' id='uncheck' style='color:red;'value='点击退回'/>"  . "</td>";
