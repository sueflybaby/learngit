...
if($numoflist<=10){	//如果总数小于等于10则全部生成
 // makeList($get_data);
  ...
  $sql ="select * from personalinformation_records WHERE 'check_rsk'='checked' ORDER BY id DESC limit $limitHead,$limitEnd";//查询
...
foreach($query_sql as $row){
  ...
  echo "
  <form name='input' action='check_ywk.php' target='_self' method='get'>";
  echo "<td>" . $num . "</td>";
  ...
  if ($row['check_ywk']=="checked")
  {
    ...
    echo "</form>";
   // echo "<form name='input' action='check_ywk.php' target='_self' method='get'>";
    echo "<input type='text' name='id' hidden='true' value='{$row['id']}'/>";
